...
def squareRepetitions(d, N):
	...
	if d not in _SQUAREGENERATORS:
		raise ValueError(
			f"square orientation is only specified for dimensions "
			f"{sorted(_SQUAREGENERATORS)}; got {d}"
		)
...
def delaunayTorus(d, N, orientation="square"):
	"""
	...
	Args:
		d (int): Dimension of the lattice.
		N (int): Scale.
		orientation (str="square"): Either `"square"` (an axis-aligned box) or
			`"rhombic"` (the lattice's own slanted box). Square is only specified
			for \(d = 2, 3, 4\); rhombic works in any dimension.
	...
	"""
	# Both cases spelled out, so a typo raises instead of quietly building a
	# rhombic torus and reporting it as whatever was asked for.
	if orientation == "rhombic": raw = delaunayRhombic(d, N)
	elif orientation == "square": raw = delaunaySquare(d, N)
	else:
		raise ValueError(
			f"unknown orientation {orientation!r}; expected 'rhombic' or 'square'"
		)
...
class Permutohedral:
	...
	def fromScale(self, dimension, scale, orientation="square", cutoff=None):
		"""
		...
		Args:
			...
			orientation (str="square"): Either `"square"` or `"rhombic"`. Square is
				only specified for \(d = 2, 3, 4\).
	...
	def fromCorners(self, corners, periodic=True, orientation="square", cutoff=None):
		"""
